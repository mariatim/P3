...
class Die:
    def __init__(self, _coords, _dots):
        self.coords = _coords
        self.dots = _dots
        ...
        self.x = [coor[0] for coor in self.coords]
        self.y = [coor[1] for coor in self.coords]

        self.xSpan = [min(self.x), max(self.x)]
        self.ySpan = [min(self.y), max(self.y)]
        self.center = [sum(self.x)/len(self.x), sum(self.y)/len(self.y)]
        self.roundedCenter = [int(c) for c in self.center]
        ...
        self.centerDistances = list(map(lambda x : dist.euclidean(x, self.center), self.coords))
